...
namespace Profiler.Dialogs
{
    internal partial class AssignDefault : Frame
    {
...
#if DEBUG
        public AssignDefault() { InitializeComponent(); }
#endif
        ...
        private void AddButton(MainWindow parent, byte selMode, byte idx)
        {
        ...
        private void AddHat(MainWindow parent, byte selMode, byte idx, ushort range)
        {
        ...
        private void AddAxis(MainWindow parent, byte selMode, byte idx, byte type, System.Collections.Generic.List<byte> axesUsed)
        {
            ...
            if (!mode.Axes.TryGetValue(idx, out Shared.ProfileModel.AxisMapModel.ModeModel.AxisModel axis))
            {
        ...
        private void NumericUpDownJ_GettingFocus(object sender, Avalonia.Input.GotFocusEventArgs args)
        {
            if (!IsLoaded)
            {
                args.Handled = true;
