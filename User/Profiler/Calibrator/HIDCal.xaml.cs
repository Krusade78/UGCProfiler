...
namespace Profiler.Calibrator
{
    internal partial class HIDCal : Frame
    {
...
#if DEBUG
        public HIDCal() { InitializeComponent(); devInfo = new(); }
#endif
        public HIDCal(Devices.DeviceInfo deviceInfo)
        {
            InitializeComponent();
        ...
        private void InsertAxes()
        {
            ...
            foreach (Shared.ProfileModel.DeviceInfo.CUsage u in devInfo.Usages)
            {
                ...
                {
                    case 0:
                        navs.Add((ushort)(x << 8), new() { Text = $"X {x++}", Tag = new AxisTempRawData() { Id = u.Id, CenterAuto = (ushort)(u.Range / 2), CenterAlt = (ushort)(u.Range / 2) } });
                        break;
                    case 1:
                        navs.Add((ushort)((y << 8) | 1), new() { Text = $"Y {y++}", Tag = new AxisTempRawData() { Id = u.Id, CenterAuto = (ushort)(u.Range / 2), CenterAlt = (ushort)(u.Range / 2) } });
                        break;
                    case 2:
                        navs.Add((ushort)((z << 8) | 2), new() { Text = $"Z {z++}", Tag = new AxisTempRawData() { Id = u.Id, CenterAuto = (ushort)(u.Range / 2), CenterAlt = (ushort)(u.Range / 2) } });
                        break;
                    case 3:
                        navs.Add((ushort)((rx << 8) | 3), new() { Text = $"Rx {rx++}", Tag = new AxisTempRawData() { Id = u.Id, CenterAuto = (ushort)(u.Range / 2), CenterAlt = (ushort)(u.Range / 2) } });
                        break;
                    case 4:
                        navs.Add((ushort)((ry << 8) | 4), new() { Text = $"Ry {ry++}", Tag = new AxisTempRawData() { Id = u.Id, CenterAuto = (ushort)(u.Range / 2), CenterAlt = (ushort)(u.Range / 2) } });
                        break;
                    case 5:
                        navs.Add((ushort)((rz << 8) | 5), new() { Text = $"Rz {rz++}", Tag = new AxisTempRawData() { Id = u.Id, CenterAuto = (ushort)(u.Range / 2), CenterAlt = (ushort)(u.Range / 2) } });
                        break;
                    case 6:
                        navs.Add((ushort)((sl << 8) | 6), new() { Text = $"Sl {sl++}", Tag = new AxisTempRawData() { Id = u.Id, CenterAuto = (ushort)(u.Range / 2), CenterAlt = (ushort)(u.Range / 2) } });
                        break;
                    default:
                        break;
                }
        ...
        private AxisTempRawData GetNavData() => (AxisTempRawData)((SelectorBarItem)lsAxes.SelectedItem).Tag;
        ...
        public void UpdateStatus(byte[] rawData, uint joy)
        {
            ...
            DispatcherQueue.TryEnqueue(() => UpdateGUI((ushort)posr, pollAxis, pollAxisGame, (ushort)(devInfo.Usages[axisId].Range / 2), GetNavData()));
        }
        ...
        private void UpdateGUI(ushort raw, ushort calibrated, ushort calibratedGame, ushort center, AxisTempRawData temprawData)
        {
            ...
            if (lastRaw != raw)
            {
                ...
                posCalGame.Margin = new Thickness(calibratedGame, 0, 0, 0);
        ...
        private void LsAxes_SelectionChanged(object sender, SelectorBarSelectionChangedEventArgs e)
        {
            Data[0].Clear();
        ...
        private void Apply()
        {
            ...
            Shared.CTypes.STLIMITS limit = new();
            if (limitJoy != null && !limitJoy.TryGetValue(GetNavData().Id, out limit))
            {
            ...
            GetNavData().CenterManualOn = tsCenter.IsOn == true;

            if (!jitters.TryGetValue(devInfo.Id, out Dictionary<byte, Shared.CTypes.STJITTER>? jitterJoy))
            {
                jitters.Add(devInfo.Id, []);
            }
            Shared.CTypes.STJITTER jitter = new();
            if (jitterJoy != null && !jitterJoy.TryGetValue(GetNavData().Id, out jitter))
            {
                jitterJoy.Add(GetNavData().Id, jitter);
            }
            jitter.Antiv = chkAntivActiva.IsOn == true ? (byte)1 : (byte)0;
