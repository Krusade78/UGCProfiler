    ...
    public partial class CtlDevices : UserControl, IDisposable
    {
        ...
        private void AddConnectedDevices()
        {
            ...
            while (API.CWinUSB.SetupDiEnumDeviceInterfaces(diDevs, IntPtr.Zero, ref hidGuid, idx++, ref diData))
            {
                ...
                System.Runtime.InteropServices.Marshal.FreeHGlobal(buf);
                if ((ninterface == null) || !ninterface.Contains("vid", StringComparison.InvariantCultureIgnoreCase) || !ninterface.Contains("pid", StringComparison.InvariantCultureIgnoreCase))
                {
                    continue;
                }
        ...
        private void WndProc(string ninterface, byte[] hidData)
        {
            if (wndProcReady)
            {
                ...
                if (hId != null)
                {
                    DispatcherQueue.TryEnqueue(() => { SetStatus(hId.Value, hidData); });
                }
            }
        }
