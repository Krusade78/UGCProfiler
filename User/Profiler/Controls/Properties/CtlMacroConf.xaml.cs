...
namespace Profiler.Controls.Properties
{
    internal sealed partial class CtlMacroConf : UserControl
    {
...
#if DEBUG
        public CtlMacroConf() { this.InitializeComponent(); parent = new(); }
#endif
        ...
        private void RadioButtonL2_Checked(object sender, RoutedEventArgs e)
        {
            if (parent.Events)
            {
                parent.Events = false;
        ...
        private void RadioButtonL1_Checked(object sender, RoutedEventArgs e)
        {
            if (parent.Events)
            {
                parent.Events = false;
        ...
        public void Init(Shared.ProfileModel.ButtonMapModel.ModeModel.ButtonModel? button)
        {
            if ((button != null) && (button.Type == 1))
            {
                ...
                panelLPS.Visibility = Visibility.Collapsed;
                panelPos.Visibility = Visibility.Visible;
            }
            if ((button != null) && (button.Type == 2))
            {
                //macros
                panelLPS.Visibility = Visibility.Visible;
