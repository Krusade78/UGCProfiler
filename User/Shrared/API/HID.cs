...
namespace API
{
    ...
    {
        public const uint HIDP_STATUS_SUCCESS = 0x110000;
        public const uint HIDP_STATUS_BUFFER_TOO_SMALL = 0xc0110007;
        ...
        [LibraryImport("hid.dll", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static partial bool HidD_SetFeature(IntPtr HidDeviceObject, byte[] ReportBuffer, uint BufferLength);


        [LibraryImport("hid.dll")]
        public static partial uint HidP_GetData(HidP_ReportType reportType, nint dataList, ref uint dataLength, nint preparsedData, byte[] report, uint reportLength);
        [LibraryImport("hid.dll")]
        private static unsafe partial uint HidP_GetData(HidP_ReportType reportType, byte* dataList, ref uint dataLength, nint preparsedData, byte[] report, uint reportLength);        
        public static uint HidP_GetData(HidP_ReportType reportType, Span<byte> dataList, ref uint dataLength, nint preparsedData, byte[] report, uint reportLength)
        {
            unsafe
            {
                fixed (byte* pDatalist = dataList)
                {
                    return HidP_GetData(reportType, pDatalist, ref dataLength, preparsedData, report, reportLength);
                }
            }
        }
        ...
        public enum HidP_ReportType :ushort
        {
            HidP_Input,
            HidP_Output,
            HidP_Feature
...
#pragma warning restore IDE1006 // Naming Styles
        ...
        {
            public ushort DataIndex;
            public HIDP_DATA_UNION Data;
        }

        [StructLayout(LayoutKind.Explicit)]
        public struct HIDP_DATA_UNION
        {
            [FieldOffset(0)]
            public uint RawValue;

            [FieldOffset(0)]
            public int On; // Para botones
        }
    }
