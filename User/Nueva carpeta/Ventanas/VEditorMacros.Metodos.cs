...
namespace Editor
{
	internal partial class VEditorMacros
	{
		...
		private void Iniciar()
		{
			...
			if (indicep > -1)
			{
				Comunes.DSPerfil.ACCIONESRow r = padre.GetData().Profile.ACCIONES.FindByidAccion((ushort)indicep);
				TextBoxNombre.Text = r.Nombre;

				List<ushort> macro = [.. r.Comandos];

				// Comprobar el check de enviar el nombre al mfd
				bool idc = false;
				if (macro.Count >= 2)
				{
					if (macro[0] == ((byte)CommandType.X52MfdTextIni + (3 << 8)))
						idc = true;
				}
				if (idc)
				{
					...
					if (macro.Count >= (st.Length + 2))
					{
						...
						if ((macro[st.Length + 1] == (byte)CommandType.X52MfdTextEnd) && nombreOk)
						{
		...
		private void CargarLista(ref List<ushort> macros)
		{
			...
			for (int i = 0; i < macros.Count; i++)
			{
				...
				{
					bloque.Add(macros[i]);
					dsMacros.MACROS.AddMACROSRow(mtr++, "", [.. bloque]);
					bloque.Clear();

				}
				else if (tipo == (byte)CommandType.X52MfdHour)
				{
					dsMacros.MACROS.AddMACROSRow(mtr++, "", [macros[i], macros[i + 1], macros[i + 2]]);
					i += 2;
				}
				...
				{
					dsMacros.MACROS.AddMACROSRow(mtr++, "", [macros[i], macros[i + 1]]);
					i++;
				}
				else if (tipo == (byte)CommandType.VkbGladiatorNxtLeds)
				{
					dsMacros.MACROS.AddMACROSRow(mtr++, "", [macros[i], macros[i + 1], macros[i + 2], macros[i + 3]]);
					i += 3;
				}
				else
				{
					dsMacros.MACROS.AddMACROSRow(mtr++, "", [macros[i]]);
				}
			}
		}
		...
		private byte GetIndice()
		{
			...
			{              
				return (byte)(((DataSetMacros.MACROSRow)dsMacros.MACROS.DefaultView[^1].Row).id + 1);
			}
			else
			{
		...
		private void Insertar(ushort[] bloque, bool separados)
		{
			...
			if (separados)
			{
				...
				{
					dsMacros.MACROS.AddMACROSRow((byte)(idInicio + c++), "", [comando]);
				}
			}
			else
			{
		...
		private void Guardar()
		{
			...
			{
				foreach (Comunes.DSPerfil.ACCIONESRow r in padre.GetData().Profile.ACCIONES.Rows)
				{
			...
			if (indicep == -1)
			{
				...
				padre.GetData().Profile.ACCIONES.AddACCIONESRow(idnuevo, TextBoxNombre.Text.Trim(), [.. macro]);
				this.DialogResult = null;
			}
			else
			{
				Comunes.DSPerfil.ACCIONESRow nr = padre.GetData().Profile.ACCIONES.FindByidAccion((ushort)indicep);
